			...
			<form action="" id="manage_user">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						...
						<div class="form-group">
							<label for="" class="control-label">Role de l'agent</label>
...
<script>
	$('[name="password"],[name="cpass"]').keyup(function(){
		...
		if(cpass == '' ||pass == ''){
			...
			if(cpass == pass){
				$('#pass_match').attr('data-status','1').html('<i class="text-success">Mot de passe correspond.</i>')
			}else{
				$('#pass_match').attr('data-status','2').html('<i class="text-danger">Mot de passe ne correspond pas.</i>')
			}
		}
	})
	...
	$('#manage_user').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast('Donnees sauvegarder avec succes.',"success");
					setTimeout(function(){
