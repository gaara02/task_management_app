				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
							<?php
							  if($stat[$row['status']] =='En cours'){
							  	echo "<span class='badge badge-secondary'>{$stat[$row['status']]}</span>";
							  }elseif($stat[$row['status']] =='débuté'){
							  	echo "<span class='badge badge-primary'>{$stat[$row['status']]}</span>";
							  }elseif($stat[$row['status']] =='En progression'){
...
<script>
	...
	_conf("Êtes vous sûr de vouloir supprimer ce projet?","delete_project",[$(this).attr('data-id')])
	})
	})
	function delete_project($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Donnee supprimé avec succes",'success')
					setTimeout(function(){
						location.reload()
					},1500)
